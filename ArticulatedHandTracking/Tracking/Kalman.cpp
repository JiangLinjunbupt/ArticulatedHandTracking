...
{
	num_shape_params = handmodel->Shape_params_num;
	if (num_shape_params == 0)
	{
		std::cout << "Num Shape Params  = 0, error!!!\n";
		return;
	}
	//根据handmodel的只设置对应的初始化的估计值，以及方差
	shape_Sigma_init = Eigen::MatrixXf::Identity(num_shape_params, num_shape_params);
	...
	estimated_values = Eigen::VectorXf::Zero(num_shape_params);
	estimated_hessian = estimated_InvShape_Sigma.transpose() * estimated_InvShape_Sigma;

	//构造函数中将 measure 相关的暂时设置为零
	measured_hessian = Eigen::MatrixXf::Zero(num_shape_params, num_shape_params);
	measured_values = Eigen::VectorXf::Zero(num_shape_params);
}
...
void Kalman::Set_measured_hessian(LinearSystem& system)
{
	...
	this->measured_hessian = 1.0f / settings->measurement_noise_value * system.lhs.block(0, 0, num_shape_params, num_shape_params);
...
void Kalman::track(LinearSystem& system, const Eigen::VectorXf& shape_params)
{
	...
	system.lhs.block(0, 0, num_shape_params, num_shape_params) += settings->weight * LHS;
	system.rhs.head(num_shape_params) += settings->weight * rhs;
}