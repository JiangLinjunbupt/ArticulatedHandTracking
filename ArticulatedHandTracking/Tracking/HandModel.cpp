...
HandModel::HandModel(Camera* _camera) :camera(_camera)
{
	...
	//pose相关的
	{
		std::vector<Eigen::Matrix4f> each_joint_pose_jacobain(this->Wrist_pose_num + this->Finger_pose_num, Eigen::Matrix4f::Zero());
	...
	//shape相关的
	{
		std::vector<Eigen::Matrix4f> each_joint_shape_jacobain(this->Shape_params_num, Eigen::Matrix4f::Zero());
	...
	//Trans相关的
	{
		...
		std::vector<Eigen::Matrix4f> each_joint_Trans_jacobain(this->Shape_params_num, Eigen::Matrix4f::Zero());
		for (int i = 0; i < this->Joints_num; ++i)
		{
	...
	pose_params_tmp.tail(Finger_pose_num) = this->Hands_mean;
	this->set_Shape_Params(Eigen::VectorXf::Zero(this->Shape_params_num));
...
void HandModel::LoadModel()
{
	...
	std::string gloveDifference_Mean_filename = ".\\Data\\gloveParamsDifferenceMean.txt";
	std::string gloveDifference_Max_Min_filename = ".\\Data\\gloveParamsDifference_maxAndmin.txt";
	std::string gloveDifference_Var_filename = ".\\Data\\gloveParamsDifference_Var.txt";
	...
	this->Load_GloveDifference_Mean(gloveDifference_Mean_filename.c_str());
	this->Load_GloveDifference_Max_Min(gloveDifference_Max_Min_filename.c_str());
	this->Load_GloveDifference_var(gloveDifference_Var_filename.c_str());
...
void HandModel::Load_GloveDifference_Mean(const char* filename)
{
	std::ifstream f;
	f.open(filename);
	if (!f.is_open())  std::cerr << "Load  GloveDifference_Mean  error,  can not open this file !!! \n";

	int num;
	f >> num;
	this->Glove_Difference_mean = Eigen::VectorXf::Zero(num);
	for (int i = 0; i < num; ++i)
		f >> this->Glove_Difference_mean(i);
	f.close();

	std::cout << "Load Glove_Difference_mean success\n";
}
void HandModel::Load_GloveDifference_Max_Min(const char* filename)
{
	std::ifstream f;
	f.open(filename);
	if (!f.is_open())  std::cerr << "Load  GloveDifference_Max_Min  error,  can not open this file !!! \n";

	int num;
	f >> num;
	this->Glove_Difference_Max = Eigen::VectorXf::Zero(num);
	this->Glove_Difference_Min = Eigen::VectorXf::Zero(num);

	for (int i = 0; i < num; ++i)
		f >> this->Glove_Difference_Max(i) >> this->Glove_Difference_Min(i);
	f.close();

	std::cout << "Load GloveDifference_Max_Min success\n";
}
void HandModel::Load_GloveDifference_var(const char* filename)
{
	std::ifstream f;
	f.open(filename);
	if (!f.is_open())  std::cerr << "Load  GloveDifference_var  error,  can not open this file !!! \n";

	int num;
	f >> num;
	this->Glove_Difference_Var = Eigen::VectorXf::Zero(num);
	for (int i = 0; i < num; ++i)
		f >> this->Glove_Difference_Var(i);
	f.close();

	std::cout << "Load GloveDifference_var success\n";
}
...
void HandModel::UpdataModel()
{
	...
	this->LBS_Updata();
	this->NormalUpdata();
	this->ComputHandVisible_IndexMap();
...
void HandModel::Trans_Matrix_Updata()
{
	...
	for (int i = 1; i < this->Joints_num; ++i)
	{
		...
		for (int j = 0; j < this->Shape_params_num; ++j)
		{
			//这里会用到 逆矩阵的求导
...
void HandModel::LBS_Updata()
{
	...
	//碰撞体变换
	{
		for (int i = 0; i < Collision_sphere.size(); ++i)
		{
			...
			this->Collision_sphere[i].Update_Center = (result2[joint_idx] * temp).head(3) + this->Pose_params.head(this->Global_position_num);
		}
	}
...
//两个雅各比函数
void HandModel::Shape_jacobain(Eigen::MatrixXf& jacobain, int vertex_id)
{
	jacobain = Eigen::MatrixXf::Zero(3, this->Shape_params_num);
...
void HandModel::Pose_jacobain(Eigen::MatrixXf& jacobain, int vertex_id)
{
	jacobain = Eigen::MatrixXf::Zero(3, this->Pose_params_num);
